...
object GenCoreDefault{
  ...
  def main(args: Array[String]) {
    ...
    val parser = new scopt.OptionParser[ArgConfig]("VexRiscvGen") {
      ...
      opt[String]("csrPluginConfig")  action { (v, c) => c.copy(csrPluginConfig = v) } text("switch between 'small', 'all', 'all2', 'linux' and 'linux-minimal' version of control and status registers configuration")
    ...
    SpinalConfig.copy(netlistFileName = argConfig.outputFile + ".v", privateNamespace=argConfig.privateNamespace).generateVerilog {
      ...
      plugins ++= List(
        ...
        new BranchPlugin(
          ...
          earlyBranch = if (!argConfig.cfu) (argConfig.earlyBranch) else true,
          catchAddressMisaligned = true
        ),
        new CsrPlugin(
          (argConfig.csrPluginConfig match {
            ...
            case "all2" => CsrPluginConfig.all(mtvecInit = argConfig.machineTrapVector).copy(ebreakGen = true)
